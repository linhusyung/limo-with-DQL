...
class environment():
    ...
    def get_state(self):
        ...
        return self.Target, self.scan_ / 8, self.pose, self.finish_pose
    ...
    def step(self, out: int):
        self.get_action(out)
        next_Target, next_scan_, next_pose, next_finish_pose = self.get_state()
        done = self.set_done()
        reward = self.set_reward()
        return next_Target, next_scan_, next_pose, next_finish_pose, reward, done
