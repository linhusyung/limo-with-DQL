...
class c():
    def __init__(self):
        ...
        self.actor.load_state_dict(torch.load('./imitate_model_1.pth'))
        self.Target_in = 50
    ...
    def get_image(self, image):
        ...
        dis = math.dist(self.amcl_pose, [0, 0])
        print(dis)
        if dis <= 1.5 and self.stop__ == True:
            self.RL = True
        cv2.imshow('img', self.cv_im)
        cv2.waitKey(1)

        if self.once:
            self.navigation_point([0, 0])
            self.once = False
        if self.RL:
            self.stop()
            action = self.sac()
    ...
    def sac(self):
        ...
        state = torch.cat(
            (self.data_to_tensor(self.Target_in).unsqueeze(0).unsqueeze(0), self.data_to_tensor(self.re_data).unsqueeze(0)),
            1)
